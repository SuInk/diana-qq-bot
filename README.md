...
WebUI 默认从根路径 `/` 进入。首次打开 `/login` 时必须由用户自行填写管理员邮箱和密码，项目不再提供或预填默认邮箱。从非本机地址完成首次设置时，还要填写 `DIANA_ADMIN_TOKEN` 作为一次性初始化凭据。浏览器使用 15 分钟 JWT access token 和 30 天轮换 refresh token；服务端保留 refresh token 哈希，因此可以在“访问设置”查看登录设备、吊销单个设备或退出其他设备。修改密码会立即吊销其他设备。`DIANA_ADMIN_TOKEN` 仍是独立的自动化 API 凭据，不会作为浏览器 Cookie 保存。
...
建议把 `Agent 工作目录` 配到独立的资料目录，不要直接指向包含密钥或生产数据的目录。`run_command` 默认不注册；只有显式配置 `DIANA_AGENT_COMMAND_ALLOWLIST` 或在 WebUI 填写命令白名单后才会启用。
...
| `DIANA_ADMIN_SECURE_COOKIES` | `false` | 经 HTTPS 反向代理访问时设为 `true`，强制管理 Cookie 使用 `Secure` |
...
| `DIANA_AGENT_COMMAND_ALLOWLIST` | 空（禁用） | Agent `run_command` 可执行命令，逗号分隔；显式配置后才注册该工具，填 `*` 允许全部命令 |
